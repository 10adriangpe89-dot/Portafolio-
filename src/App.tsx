...
const EXPERIENCE: ExperienceItem[] = [
  {
    ...
    color: "#00c9ff",
    items: [
      "Digitalización Institucional: Sistema de Control Escolar (.NET/C#) — migración digital del 100% de expedientes.",
      "Reducción del 40% en tiempos de inscripción mediante automatización de flujos administrativos.",
      "Inventarios PHP/Laravel/MariaDB: Trazabilidad exacta de 200+ activos tecnológicos.",
      "Optimización DB: Consultas de reportes críticos en menos de 1 segundo.",
    ],
  },
  {
    period: "Nov 2024 – Ago 2025",
    ...
    color: "#e8ff5a",
    items: [
      "99% de disponibilidad operativa en Plazas Comunitarias mediante mantenimiento a nivel componente.",
      "Soporte Nivel 3 en sistemas SIGA y SASA, garantizando continuidad educativa en la zona.",
      "Infraestructura rural: Despliegue de red en áreas remotas para acceso a plataformas educativas.",
    ],
  },
  {
    period: "Oct 2018 – Sep 2024",
    ...
    color: "#ff5fa0",
    items: [
      "35% de reducción en gasto de hardware mediante taller técnico interno de rehabilitación.",
      "Nómina Municipal: Automatización del pago de 500+ empleados con integración fiscal.",
      "Seguridad perimetral de servidores institucionales y cumplimiento ante DeclaraNet.",
    ],
  },
];
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "2rem" }}>
          ...
          <div style={{ background: `${project.color}10`, borderLeft: `2px solid ${project.color}`, padding: "0.75rem 1rem", marginBottom: "1.5rem", fontSize: "0.78rem", color: project.color, lineHeight: 1.6 }}>
            {project.impact}
          </div>
...
function TypewriterTitle({ style }: { style: React.CSSProperties }) {
  const [lineIdx, setLineIdx] = useState(0);
  const [charIdx, setCharIdx] = useState(0);
  const [done, setDone] = useState(false);
  ...
  useEffect(() => {
    if (done) return;
    const current = TITLE_LINES[lineIdx];
    if (charIdx < current.text.length) {
      const t = setTimeout(() => setCharIdx((c) => c + 1), 55);
      return () => clearTimeout(t);
    }
    if (lineIdx < TITLE_LINES.length - 1) {
      const t = setTimeout(() => { setLineIdx((l) => l + 1); setCharIdx(0); }, 320);
      return () => clearTimeout(t);
    }
    setDone(true);
        ...
        {TITLE_LINES.map((line, i) => {
          ...
          return (
            <span key={i} style={{ display: "block", color: line.color, minHeight: "1.1em" }}>
              {displayed}
              {showCursor && (
                <span style={{ display: "inline-block", width: 3, height: "0.85em", background: "#00c9ff", marginLeft: 4, verticalAlign: "middle" }} />
              )}
            </span>
          );
        })}
...
function Hero() {
  ...
  return (
    <section id="hero" style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "0 3rem", position: "relative", overflow: "hidden" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", width: "100%", position: "relative", zIndex: 1, paddingTop: "5rem" }}>
        <div style={{ ...s(0), display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "rgba(0,201,255,0.1)", border: "1px solid rgba(0,201,255,0.25)", color: "#00c9ff", padding: "0.35rem 1rem", borderRadius: 100, fontSize: "0.68rem", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "1.5rem" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00c9ff" }} />
          Disponible para nuevos proyectos
        </div>
        <TypewriterTitle style={s(0.1)} />
        <p style={{ ...s(0.2), fontSize: "0.9rem", color: "#6b82a8", maxWidth: 460, lineHeight: 1.8, marginBottom: "2.5rem" }}>
          Especialista TI con <strong style={{ color: "#d6e4ff" }}>+8 años</strong> transformando instituciones gubernamentales mediante desarrollo Full Stack, infraestructura crítica y automatización.
        </p>
        <div style={{ ...s(0.3), display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "3.5rem" }}>
          <a href="#projects" style={{ background: "#00c9ff", color: "#000", padding: "0.85rem 2rem", borderRadius: 4, fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.83rem", textDecoration: "none", textTransform: "uppercase" }}>Ver proyectos →</a>
          <a href="#contact" style={{ border: "1px solid #1e2d45", color: "#d6e4ff", padding: "0.85rem 2rem", borderRadius: 4, fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.83rem", textDecoration: "none", textTransform: "uppercase" }}>Contactar</a>
        </div>
        <div style={{ ...s(0.4), display: "flex", gap: "3rem", flexWrap: "wrap", paddingTop: "2.5rem", borderTop: "1px solid #1e2d45" }}>
          {[["8+", "Años exp."], ["500+", "Usuarios"], ["200+", "Activos TI"], ["35%", "Ahorro HW"]].map(([v, l]) => (
            <div key={l}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "2rem", fontWeight: 800, color: "#e8ff5a" }}>{v}</div>
              <div style={{ fontSize: "0.68rem", color: "#6b82a8", textTransform: "uppercase", marginTop: "0.3rem" }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
...
function Skills() {
  ...
  return (
    <section id="skills" style={{ padding: "6rem 3rem", background: "#0f1623" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <SectionLabel>Especialidades</SectionLabel>
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 4vw, 3.2rem)", marginBottom: "3rem", color: "#fff" }}>Stack técnico</h2>
        <div ref={ref} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: 1, background: "#1e2d45", border: "1px solid #1e2d45", opacity: visible ? 1 : 0, transition: "0.6s ease" }}>
          {SKILLS.map((sk) => (
            <div key={sk.name} style={{ background: "#0f1623", padding: "2.2rem" }}>
              <div style={{ fontSize: "1.3rem", marginBottom: "0.8rem" }}>{sk.icon}</div>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, color: "#fff", marginBottom: "0.6rem" }}>{sk.name}</div>
              <p style={{ fontSize: "0.74rem", color: "#6b82a8", lineHeight: 1.7, marginBottom: "1rem" }}>{sk.desc}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                {sk.tags.map((t) => <Tag key={t} accent>{t}</Tag>)}
              </div>
            </div>
...
function Experience() {
  ...
  return (
    <section id="experience" style={{ padding: "6rem 3rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <SectionLabel>Trayectoria</SectionLabel>
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 4vw, 3.2rem)", marginBottom: "3.5rem", color: "#fff" }}>Experiencia</h2>
        <div ref={ref} style={{ position: "relative", opacity: visible ? 1 : 0, transition: "0.6s ease" }}>
          <div style={{ position: "absolute", left: 170, top: 0, bottom: 0, width: 1, background: "#1e2d45" }} />
          {EXPERIENCE.map((ex, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "170px 1fr", gap: "0 3rem", marginBottom: "3.5rem", position: "relative" }}>
              <div style={{ position: "absolute", left: 167, top: 8, width: 7, height: 7, borderRadius: "50%", background: ex.color }} />
              <div style={{ textAlign: "right", paddingRight: "2.5rem", fontSize: "0.7rem", color: "#6b82a8" }}>{ex.period}</div>
              <div>
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, color: "#fff", marginBottom: "0.25rem" }}>{ex.role}</div>
                <div style={{ fontSize: "0.72rem", color: ex.color, textTransform: "uppercase", marginBottom: "1rem" }}>{ex.company}</div>
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {ex.items.map((item, j) => (
                    <li key={j} style={{ fontSize: "0.77rem", color: "#6b82a8", lineHeight: 1.7 }}>
                      <span style={{ color: ex.color, marginRight: "0.5rem" }}>›</span> {item}
                    </li>
                  ))}
                </ul>
              </div>
...
function Projects({ onSelect }: { onSelect: (p: Project) => void }) {
  ...
  return (
    <section id="projects" style={{ padding: "6rem 3rem", background: "#0f1623" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <SectionLabel>Portafolio</SectionLabel>
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 4vw, 3.2rem)", marginBottom: "3rem", color: "#fff" }}>Proyectos</h2>
        <div ref={ref} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 1, background: "#1e2d45", border: "1px solid #1e2d45", opacity: visible ? 1 : 0, transition: "0.6s ease" }}>
          {PROJECTS.map((p) => (
            <div key={p.id} onClick={() => onSelect(p)} style={{ background: "#0f1623", padding: "2.2rem", cursor: "pointer" }}>
              <div style={{ fontSize: "0.68rem", color: "#6b82a8", marginBottom: "1.2rem" }}>Proyecto {p.num}</div>
              <div style={{ width: 36, height: 3, background: p.color, borderRadius: 2, marginBottom: "1rem" }} />
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, color: "#fff", marginBottom: "0.7rem" }}>{p.title}</div>
              <p style={{ fontSize: "0.74rem", color: "#6b82a8", marginBottom: "1rem" }}>{p.desc}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem", marginBottom: "1.5rem" }}>
                {p.tech.map((t) => <Tag key={t}>{t}</Tag>)}
              </div>
              <div style={{ fontSize: "0.72rem", color: p.color }}>Ver demo →</div>
            </div>
          ))}
        </div>
      </div>
...
function Contact() {
  return (
    <section id="contact" style={{ padding: "6rem 3rem" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <SectionLabel>Contacto</SectionLabel>
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 4vw, 3.2rem)", marginBottom: "3rem", color: "#fff" }}>Hablemos</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "4rem" }}>
          <div>
            <p style={{ fontSize: "1.2rem", color: "#00c9ff", marginBottom: "1.5rem", fontStyle: "italic" }}>"Infraestructura que resiste. Código que escala."</p>
            <p style={{ fontSize: "0.8rem", color: "#6b82a8", lineHeight: 1.8 }}>Disponible para posiciones full-time, consultoría o proyectos. Movilidad regional inmediata.</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <a href="mailto:10adriangpe89@gmail.com" style={{ display: "block", padding: "1rem", border: "1px solid #1e2d45", color: "#d6e4ff", textDecoration: "none" }}>Email: 10adriangpe89@gmail.com</a>
            <a href="tel:9961109967" style={{ display: "block", padding: "1rem", border: "1px solid #1e2d45", color: "#d6e4ff", textDecoration: "none" }}>Tel: 996-110-9967</a>
          </div>
        </div>
      </div>
    </section>
...
function Nav() {
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "1.5rem 3rem", display: "flex", justifyContent: "space-between", background: "rgba(9,13,20,0.8)", backdropFilter: "blur(10px)" }}>
      <span style={{ color: "#00c9ff", fontWeight: 800 }}>AH // DEV</span>
      <div style={{ display: "flex", gap: "2rem" }}>
        {["#skills", "#experience", "#projects", "#contact"].map(h => <a key={h} href={h} style={{ color: "#6b82a8", textDecoration: "none", fontSize: "0.7rem" }}>{h.replace('#','')}</a>)}
      </div>
    </nav>
  );
}
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #090d14; color: #d6e4ff; font-family: 'DM Mono', monospace; scroll-behavior: smooth; }
      `}</style>
      <Nav />
      <Hero />
      <Skills />
